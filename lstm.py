...
class LSTM:
    ...
    def train(self, x, epochs=3, batch_size=32, log_interval=100):
        ...
        self.test_program = fluid.Program().clone(for_test=True)
...
if __name__ == "__main__":
    ...
    x_train = list(open(folder_prefix + "amazon-reviews-train-no-stop.txt", 'r').readlines())
    x_test = list(open(folder_prefix + "amazon-reviews-test-no-stop.txt", 'r').readlines())
    #x_train = list(open('ptb.valid.txt', 'rb').readlines())
    #x_test = list(open('ptb.valid.txt', 'rb').readlines())
    ...
    x_test = file_to_ids(x_text[-test_size:], vocabulary)
    x_train = file_to_ids(x_text[:-test_size], vocabulary)
    ...
    lstm_test.train(x_train, epochs=1, batch_size=32, log_interval=1)
    lstm_test.evaluate(x_test)
